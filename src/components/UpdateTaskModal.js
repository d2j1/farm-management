...
function RepeatStepper({ value, onDecrement, onIncrement, unit, t }) {
 return (
 <View className="flex-row items-center justify-between bg-slate-50 p-3 rounded-2xl border border-slate-100 mt-4">
 ...
 <Pressable
 onPress={onDecrement}
 className="w-10 h-10 items-center justify-center rounded-lg bg-white shadow-sm border border-slate-200 "
 >
...
function RecurringInfoBanner({ interval, unit, endDate, t }) {
 ...
 <Text className="text-slate-700 text-sm leading-relaxed font-medium flex-1">
 {t('repeatInfo')
...
export default function UpdateTaskModal({ visible, onClose, onSave, taskData }) {
 ...
 <Text className="text-slate-700 text-sm font-bold uppercase tracking-[0.1em]">
 {t('taskNameLabel')}
 </Text>
 <TextInput
 className={`rounded-xl border ${
 error ? 'border-red-500' : 'border-slate-200 '
 } bg-slate-50 text-slate-900 h-14 px-4 placeholder:text-slate-400`}
 ...
 <View className="mb-6">
 <Text className="text-slate-700 text-sm font-bold uppercase tracking-[0.1em] mb-3 block">
 {t('duration')}
 </Text>
 <SegmentedControl
